...
export function parseWorkflow(workflowPath: string): Workflow {
	...
	const jobs = Object.entries(parsed?.jobs ?? {}).map(([jobId, job]) => parseJob(jobId, job));
...
function parseJob(jobId: string, job: JobYaml): Job {
	const steps = (job.steps ?? []).map((step, index) => parseStep(jobId, step, index));
...
function parseWorkflowEvents(trigger: WorkflowYaml["on"]): string[] {
	if (!trigger) {
		return [];
	}
